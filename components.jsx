...
function IconMenu({size=20}) {
  return <svg width={size} height={size} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5"><line x1="3" y1="5" x2="17" y2="5"/><line x1="3" y1="10" x2="17" y2="10"/><line x1="3" y1="15" x2="17" y2="15"/></svg>;
}
function IconClose({size=20}) {
  return <svg width={size} height={size} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5"><line x1="4" y1="4" x2="16" y2="16"/><line x1="16" y1="4" x2="4" y2="16"/></svg>;
}
...
function Nav({ page, navigate }) {
  ...
  const handleLink = (e, l) => {
    e.preventDefault();
    setMenuOpen(false);
    if (l === "Corporate Orders") { navigate('b2b'); return; }
    if (l === 'Fabrics') {
    ...
    <>
      <div className="nav">
        ...
        <div className="nav-links">
          {links.map(l => {
            ...
            return (
              <a key={l} href="#" onClick={(e) => handleLink(e, l)}
      ...
      <div className={"mobile-drawer" + (menuOpen ? ' open' : '')} aria-hidden={!menuOpen}>
        <div className="mobile-drawer-header">
          <BrandLogo/>
          <button className="mobile-drawer-close" aria-label="close menu" onClick={() => setMenuOpen(false)}><IconClose size={22}/></button>
        </div>
        <nav className="mobile-drawer-links">
          {links.map(l => (
            <a key={l} href="#" onClick={(e) => handleLink(e, l)}
              className={l === "Corporate Orders" && page === 'b2b' ? 'active' : (l==='New In' && page==='home' ? 'active':'')}>{l}</a>
          ))}
        </nav>
        <div className="mobile-drawer-footer">
          <button aria-label="search" className="mobile-drawer-icon"><IconSearch size={20}/></button>
          <button aria-label="bag" className="mobile-drawer-icon" style={{position:'relative'}}><IconBag size={20}/><span style={{position:'absolute',top:0,right:0,width:14,height:14,borderRadius:'50%',background:'var(--chocolate)',color:'#fff',fontSize:9,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Funnel Sans'}}>2</span></button>
        </div>
      </div>
